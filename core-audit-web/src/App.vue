  ...
  <div class="app">
    <aside class="sidebar">
      <div class="brand" @click="router.push('/')">
        <span class="brand-icon"><FontAwesomeIcon icon="layer-group" /></span>
        <span class="brand-text">core-audit</span>
      </div>
      <nav class="nav">
        <router-link to="/" class="nav-item" :class="{ active: isActive('/') }">
          <span class="nav-icon"><FontAwesomeIcon icon="gauge" /></span>
          <span>概览</span>
        </router-link>
        <router-link to="/events" class="nav-item" :class="{ active: isActive('/events') || currentRoute.path.startsWith('/events/') }">
          <span class="nav-icon"><FontAwesomeIcon icon="bars" /></span>
          <span>审计记录</span>
        </router-link>
        <router-link to="/timeline/_search" class="nav-item" :class="{ active: currentRoute.path.startsWith('/timeline/') || currentRoute.path.startsWith('/object/') || currentRoute.path.startsWith('/operator/') || currentRoute.path.startsWith('/trace/') }">
